...
|---|---|---|
| M0 — Foundation | **In progress** | Build, editor seams, bridge/RPC, release, SSH channel evidence, generated-key Android-shaped fixed-RPC proof, API 36 x86_64 KVM device smoke, bounded Android Keystore/software RSA-to-Russh wire-signature probes, and the real local `chooshd` private-socket frame test pass. The loopback OpenSSH acceptance runner remains environment-blocked when `/run/sshd` is unavailable. The shipped M0 UI is a Java/View connection-status screen, not the future Compose shell. M0-R5/R6 still need the real JVM callback/socket-to-`chooshd` proof; M0-R7/R15 remain blocked on terminal provenance and device implementation. |
...
| M2 — Agents and notifications | **In progress** | Core event spool, adapter normalization, fixed-argv bounded hook ingestion, redacted waiting-notification projection, exact-target notification activation, Android constructor-injected notification projection, and bounded replay/ack tests pass headlessly. A constructor-injected Android notification lifecycle seam now owns source start/stop and clears projections on teardown; `AndroidNotificationSink` now provides API-safe channel creation and deterministic keyed `NotificationManager` posting/clearing. SSH-backed end-to-end subscription and device acceptance remain future gates. |
