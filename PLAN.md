...
| M2 — Agents and notifications | **In progress** | Core event spool, adapter normalization, fixed-argv bounded hook ingestion, redacted waiting-notification projection, exact-target notification activation, Android constructor-injected notification lifecycle, API-safe keyed `NotificationManager` sink, and bounded replay/ack tests pass headlessly. Versioned `events.subscribe-v1` and `events.ack-v1` RPCs now provide bounded replay or `snapshot_required` responses with typed validation. Device acceptance and full hook-to-notification integration remain future gates. |
