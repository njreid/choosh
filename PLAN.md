...
| M6 — Public 1.0 release | **In progress** | Release discovery now has a deterministic artifact manifest gate (`scripts/check-release-reproducibility.sh`) covering every bounded fixture asset and rejecting tampering. Upgrade acceptance now proves same-version idempotence and restart-safe retry after a health rollback without stale staging/previous state. Device/accessibility, migration, hardened host updates, and all prior gates remain. |
